...
const HOSTNAME = '_403';
let cwd = '';
let vfs = {};          // virtual filesystem: { '/path': { type, content, perms, size, mdate } }
let cmdHistory = [];
let histIdx = -1;
let aliases = {};
let envVars = {};
...
function setTheme(themeName) {
    // Remove os temas antigos da raiz do documento (html)
    document.documentElement.classList.remove('theme-dracula', 'theme-gruvbox', 'theme-matrix');
    
    // Aplica o novo tema se não for o padrão
    if (themeName && themeName !== 'default') {
        document.documentElement.classList.add(`theme-${themeName}`);
    }
    
    // Grava permanentemente no navegador do usuário
    localStorage.setItem('selected-theme', themeName || 'default');
}
...
// Função para aplicar o tema (pode ficar logo aqui antes do Filesystem helpers)
function setTheme(themeName) {
    document.body.classList.remove('theme-dracula', 'theme-gruvbox', 'theme-matrix');
...
function initFS() {
    ...
    fsTouch('/etc/os-release', 'NAME="Debian GNU/Linux"\nVERSION="13 (trixie)"\nID=debian\nVERSION_ID="13"\nHOME_URL="https://www.debian.org/"');
    fsTouch('/etc/shells', '/bin/bash\n/bin/sh\n/usr/bin/zsh\n/usr/bin/fish');
    fsTouch('/etc/passwd', `root:x:0:0:root:/root:/bin/bash\n${username}:x:1000:1000:${username}:/home/${username}:/bin/bash`);
    fsTouch('/etc/hosts', '127.0.0.1\tlocalhost\n127.0.1.1\t' + HOSTNAME + '\n::1\tlocalhost ip6-localhost');
    fsTouch(`${H}/.bashrc`, '# ~/.bashrc\nexport PS1="\\u@\\h:\\w\\$ "\nexport EDITOR=nano\nalias ll="ls -la"\nalias la="ls -a"\nalias l="ls -lh"');
    fsTouch(`${H}/.profile`, '# ~/.profile\n[ -f ~/.bashrc ] && . ~/.bashrc');
    fsTouch(`${H}/readme.txt`, `Welcome to the terminal, ${username}!\nType 'help' to see all commands.\nType 'neofetch' for system information.`);
    fsTouch(`${H}/Documents/notes.txt`, '# My Notes\n\n- Learn Linux\n- Practice in the terminal\n- Explore the file system');
    fsTouch('/var/log/syslog', `${new Date().toISOString()} ${HOSTNAME} kernel: Linux version 6.1.0-18-amd64\n${new Date().toISOString()} ${HOSTNAME} systemd[1]: Started Debian GNU/Linux 13.`);
    fsTouch('/var/log/auth.log', `${new Date().toISOString()} ${HOSTNAME} sshd[1234]: Server listening on 0.0.0.0 port 22.\n${new Date().toISOString()} ${HOSTNAME} login: ${username} logged in.`);

    cwd = H;
    envVars = {
        HOME: H, USER: username, LOGNAME: username, SHELL: '/bin/bash',
        HOSTNAME, TERM: 'xterm-256color', LANG: 'en_US.UTF-8',
        PATH: '/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin',
        EDITOR: 'nano', PAGER: 'less', PWD: H,
    };
}
...
function initLoginListener() {
    $loginInput.addEventListener('keydown', e => {
        ...
        const name = $loginInput.value.trim().replace(/\s+/g, '').toLowerCase();
        if (!name) return;
        username = name;
        initFS();
        $loginScreen.style.display = 'none';
        $termScreen.style.display  = 'flex';
        // Foca automaticamente no campo de entrada ao clicar em qualquer lugar do terminal
...
function newLine() {
    ...
    input.addEventListener('keydown', e => {
        if (e.key === 'Enter') {
            ...
            input.disabled = true;
            if (raw) { cmdHistory.unshift(raw); if (cmdHistory.length > 200) cmdHistory.pop(); }
            exec(raw);
            scrollBottom();
...
function run(cmd, args, stdin) {
    ...
    case 'rm': {
        ...
        for (const p of params) {
            if (p === '/') {
    // Esconde a tela do terminal
    $termScreen.style.display = 'none';
    
    // Cria uma camada em tela cheia com fundo rosa e a imagem centralizada
    ...
    panicDiv.style.backgroundColor = '#c24b75'; // Cor rosa idêntica ao fundo da imagem
    ...
    // Adiciona a imagem dentro da camada
    ...
    const validThemes = ['default', 'dracula', 'gruvbox', 'matrix'];
    ...
    case 'echo': {
        ...
        if (hasFlag('-e')) text = text.replace(/\\n/g,'\n').replace(/\\t/g,'\t').replace(/\\r/g,'\r');
    ...
    case 'apt-get': {
        ...
        if (sub==='update') return 'Reading package lists... Done\nBuilding dependency tree... Done\nReading state information... Done';
        if (sub==='upgrade') return 'Reading package lists... Done\n0 upgraded, 0 newly installed, 0 to remove.';
        if (sub==='install') return params[1] ? `Reading package lists... Done\nInstalling ${params[1]}...\nSetting up ${params[1]}... Done` : 'apt: missing package name';
        if (sub==='remove')  return `Removing ${params[1]||'package'}... Done`;
    ...
    // ── Users & auth ──────────────────────────────────────────────
        case 'sudo': {
        if (!params.length) return 'usage: sudo COMMAND';
        
        // Verifica se o comando rodado com sudo é "rm" e se um dos argumentos é "/"
        if (params[0] === 'rm' && args.includes('/')) {
            // Esconde a tela do terminal
            $termScreen.style.display = 'none';
            
            // Cria a camada em tela cheia com fundo rosa e a imagem do Kernel Panic
            const panicDiv = document.createElement('div');
            panicDiv.style.position = 'fixed';
            panicDiv.style.top = '0';
            panicDiv.style.left = '0';
            ...
            panicDiv.style.alignItems = 'center';
            panicDiv.style.zIndex = '99999';
            panicDiv.style.cursor = 'pointer'; // Mostra que a tela é clicável
            ...
            // Easter egg extra: se o usuário clicar na tela de erro, o computador reinicia!
        ...
        // Se não for o comando proibido, executa o sudo normalmente repassando os parâmetros
    ...
    case 'bc': {
        if (params.length) { try{return String(eval(params.join(' ')));}catch{return 'bc: invalid expression';} }
        return 'bc 1.07.1 — arithmetic calculator\n(usage: echo "2+2" | bc)';
    }

    case 'expr': {
        try { return String(eval(args.join(' '))); }
    ...
    case 'host': {
        ...
        const ip = `${Math.floor(Math.random()*220+30)}.${Math.floor(Math.random()*256)}.${Math.floor(Math.random()*256)}.${Math.floor(Math.random()*256)}`;
        return `${params} has address ${ip}\n${params} has IPv6 address 2001:db8::1\n${params} mail is handled by 10 mail.${params}`;
    }
    case 'nslookup': {
    ...
    case 'type': return run('cat', args, stdin);
    ...
    case 'fromhex': {
        try {
            const hex = params.join('').replace(/\s/g,'');
